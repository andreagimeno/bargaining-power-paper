...
AgreementScores <- AgreementScores %>%
  dplyr::select(-c(...1, session.x, NVotesAll.x, NVotesAll.y, StateAbb, IdealPointAll.x, IdealPointAll.y, IdealPointDistance))
...
wgi <- wgi %>%
  ...
  dplyr::select(-rl, -va)  # rule of law / voice & accountability: not part of the s_inst dimension
...
# Source: World Bank WITS, Export Value Added (EVAD) country stats
# https://wits.worldbank.org/analyticaldata/evad-countrystats.aspx
...
CRS_Data <- CRS_Data %>%
  dplyr::select(-any_of(c("eff_tariff", "mfn_rate",
  ...
  tidyr::fill(eff_tariff, mfn_rate, .direction = "downup") %>%
  ungroup() %>%
